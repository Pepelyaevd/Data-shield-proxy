...
**UI-лаунчер (для людей).** Кнопка «Открыть Claude (под защитой)» вместо терминала:
```bash
make webui   # браузерный UI (рекомендуется), без зависимостей, поверх launch.py
make ui      # запасной вариант на Tkinter (на системном Tk 8.5/macOS может не рисовать)
```
Кнопка открывает Claude Code в терминале с уже внедрёнными прокси/CA/identity;
кнопка «Проверить перехват» шлёт тест-секрет и показывает, что инспекция сработала.
...
## Снижение ложных срабатываний (false positives)

Из практики: доминирующий шум — `high_entropy_string` на служебных
идентификаторах и egress телеметрии агентов. Что сделано «из коробки»:

- **Телеметрия не инспектируется.** Egress на observability/аналитику
  (datadog, sentry, statsig, launchdarkly, amplitude, …) фиксируется для аудита
  (объём, атрибуция), но **не сканируется** DLP. Включить скан:
  `DSP_SCAN_TELEMETRY=true`. Список — `catalog.TELEMETRY_DOMAINS`.
- **Энтропийный детектор ужат:** пороги подняты (энтропия ≥ 4.2, длина ≥ 24) и
  исключён структурный шум — git-SHA, md5/sha-хэши, UUID, trace/span-id, длинные
  числа. Настоящие ключи ловятся сигнатурами, а не энтропией.
- **Дедуп в рамках одного скана:** один и тот же секрет не даёт дублей. При
  пересечении спанов «широкие» находки (`high_entropy_string`,
  `credential_assignment`) подавляются более специфичной сигнатурой/PII.
- **Известные провайдеры сканируются по промпту, а не по всему телу** — парсеры
  извлекают текст сообщений, служебные поля (id, заголовки, usage) не попадают
  в скан.
