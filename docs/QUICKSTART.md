...
**UI-лаунчер (для людей).** Вместо терминала — окно с кнопкой «Открыть Claude
(под защитой)» (Tkinter, без зависимостей, поверх `launch.py`):
```bash
make ui   # или: python3 launcher/ui.py
```
